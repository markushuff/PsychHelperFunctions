#' Lineplot within-subject error bars
...
lineplot.CI.within <- function(x.factor, response, group = NULL, data = NULL, sid = NULL, subset = NULL, conf.level = 0.95, within.ci.method = c("baguley", "morey", "cousineau"), ...)
{
  ...
  #lineplot.CI(x.factor=x.factor,response=response.normalized,group=group,data=withinCI$daten,fun=withinCI$fun,ci.fun=withinCI$ci.fun,...)
  # Hack: Neues sciplot: fun nicht explizit definieren (eval Problem), sondern Annahme, dass Mittelwert ist
  sciplot::lineplot.CI(x.factor=x.factor,response=response.normalized,group=group,data=withinCI$daten,ci.fun=withinCI$ci.fun,...)
}
